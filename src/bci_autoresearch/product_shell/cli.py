...
def build_doctor_report(*, repo_root: Path, host: str, port: int) -> dict[str, Any]:
    ...
    report = {
        ...
        "structure_sandbox_runner": {
            "ok": bool(custom_structure_runner or codex_path),
            "custom_runner": custom_structure_runner or None,
            "codex_path": codex_path,
            "active": custom_structure_runner or codex_path,
...
def format_doctor_report(report: dict[str, Any], *, windows_only: bool = False, linux_only: bool = False) -> str:
    ...
    rows = [
        ...
        f"- structure sandbox runner：{'通过' if report['structure_sandbox_runner']['ok'] else '未找到 codex 或自定义 runner'}",
    ...
    *,
    ...
    prompt = "\n".join(
        [
            ...
            "AutoBCI 可以处理脑接口、脑电、图像、跨模态和普通机器学习研究；不要默认把所有任务解释成脑电任务。",
            "当用户说图像任务、图片任务、ship / not-ship、船只 / 非船、是不是船，但没有明确要求使用脑电或比较脑电时，必须按 image-only 纯图像任务理解。",
            "即使用户提到一个看起来完整的研究方向，也要先当作讨论。只有用户明确说“生成 Program / 写 Program / 重写 Program / 按当前版本生成”时，才选择 draft_program。",
            "image-only 纯图像任务在生成 Program 时不要追问是否采集脑电数据；只能追问或记录图片目录、标签来源、数据划分和成功指标。",
            "只有用户明确说比较图像和脑电、使用 EEG、脑电识别、跨模态比较时，才把任务解释成 cross-modal。",
            "",
            "可用工具名：",

...
TRACK_MANIFEST = ROOT / "tools" / "autoresearch" / "tracks.gait_phase_eeg.json"
...
def load_tracks() -> list[dict[str, Any]]:
    payload = json.loads(TRACK_MANIFEST.read_text(encoding="utf-8"))
...
def build_baseline_row(*, campaign_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "next_step": "开始 timing scan smoke：TCN/GRU × 4 个窗长 × 4 个时延。",
...
def write_runtime_status(
    *,
    ...
    active_track_payload = next(
        (track for track in load_tracks() if str(track.get("track_id") or "") == active_track_id),
        {},
    )
    ...
    runtime.update(
        {
            ...
            "validation_summary": "固定 0717/0719 临时标签，只比较 TCN/GRU 在不同窗长与固定全局时延下的二分类效果。",

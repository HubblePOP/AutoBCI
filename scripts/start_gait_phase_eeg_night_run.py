...
def _sync_benchmark_files(paths) -> Path:
    ...
    for relative in ("tools/autoresearch/program.md", "tools/autoresearch/program.current.md", "tools/autoresearch/tracks.current.json"):
        ...
        if source.exists():
            ...
            shutil.copy2(source, target)
    _sync_file(ROOT / "tools" / "autoresearch" / "program.gait_phase.eeg.md", ROOT / "tools" / "autoresearch" / "program.md")
    _sync_file(ROOT / "tools" / "autoresearch" / "program.gait_phase.eeg.current.md", ROOT / "tools" / "autoresearch" / "program.current.md")
    _sync_file(ROOT / "tools" / "autoresearch" / "tracks.gait_phase_eeg.json", ROOT / "tools" / "autoresearch" / "tracks.current.json")
    return backup_dir
...
def main() -> None:
    ...
    backup_dir = _sync_benchmark_files(paths)
    ...
    runtime.update(
        {
            ...
            "validation_summary": "固定 0717/0719 步态临时标签，只比较 TCN/GRU 在不同窗长和固定全局时延下的二分类效果。",

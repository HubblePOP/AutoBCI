...
1. Read `memory/docs/dev_pack_2026_04_20/08_LOCAL_AGENT_HANDOFF/TUI_PTY_HARNESS.md`.
...
5. If the change touches provider, Pi runtime, Intake Agent behavior, `/model`, or `/plan`, PTY is not enough. Also run the scenario smoke:

```bash
PYTHONPATH=src python -m bci_autoresearch.product_shell.cli smoke intake-llm --provider mimo --model mimo-v2-pro --json
```

Use the currently configured real provider/model. Missing keys, bad model names, incompatible JSON, or runtime errors must fail visibly; do not replace them with a local substitute.

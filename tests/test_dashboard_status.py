...
class DashboardStatusTests(unittest.TestCase):
    ...
    def test_dashboard_index_declares_sota_source_filter_and_defaults_to_all_history(self) -> None:
        html = Path("/Users/mac/Code/AutoBci/dashboard/index.html").read_text()

        self.assertIn('id="sota-time-range"', html)
        self.assertIn('option value="all" selected', html)
        self.assertIn('id="sota-source-filter"', html)
        self.assertIn('option value="all" selected>全部', html)

...
class DashboardStatusTests(unittest.TestCase):
    ...
    def test_build_operator_summary_surfaces_manual_stop_loss_semantics(self) -> None:
        summary = build_operator_summary(
            ...
            autoresearch_status={
                ...
                "candidate": {
                    "run_id": "overnight-2026-04-08-struct-r05-canonical_mainline_feature_lstm-iter-006",
                    "track_id": "canonical_mainline_feature_lstm",
                    "stage": "smoke",
    ...
    def test_build_mainline_progress_exposes_branch_overlays_without_merging_them_into_mainline(self) -> None:
        progress_rows = [
            {
                "run_id": "canonical-mainline-001",
        ...
        mainline = build_mainline_progress({}, progress_rows)
        primary_overlays = mainline["plots"]["primary"]["overlays"]
        rmse_overlays = mainline["plots"]["val_rmse"]["overlays"]

        self.assertEqual(mainline["row_count"], 1)
        self.assertEqual(mainline["plots"]["primary"]["total_points"], 1)
        self.assertEqual([overlay["group_id"] for overlay in primary_overlays], ["relative_origin_xyz", "relative_origin_xyz_upper_bound"])
        self.assertEqual([overlay["label"] for overlay in primary_overlays], ["相对 RSCA 三方向坐标", "相对 RSCA 同试次上限参考"])
        self.assertEqual([point["value"] for point in primary_overlays[0]["points"]], [0.3641])
        self.assertEqual([point["value"] for point in rmse_overlays[1]["points"]], [34.9])
